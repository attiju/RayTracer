...
FilmPixel &Film::operator[](const Point2i &pixel) {
  return pixels[(pixel.x - pixel_bounds.p_min.x) +
      (pixel.y - pixel_bounds.p_min.y) * (pixel_bounds.p_max.x - pixel_bounds.p_min.x)];
}
...
void Film::merge(std::unique_ptr<FilmTile> tile) {
  ...
  for (Point2i pixel : tile->bounds) {
    const FilmPixel &tile_pixel = tile->operator[](pixel);
    FilmPixel &film_pixel = this->operator[](pixel);
    film_pixel.color += tile_pixel.color;
